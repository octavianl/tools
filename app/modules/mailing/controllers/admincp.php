...
class Admincp extends Admincp_Controller {
	function __construct()
	{
		...
		$this->admin_navigation->parent_active('mailing');
		
		$this->admin_navigation->module_link('Add New Email',site_url('admincp/mailing/mail_add'));
	...
	function send() {
		...
		// get templates
		$this->load->model('mailing/email_template_model');
	...
	function post_send() {
		...
		if ($this->form_validation->run() === FALSE) {
			...
			return redirect('admincp/mailing/send');
		}
	...
	*/
	function index()
	{
		...
		$this->dataset->base_url(site_url('admincp/mailing'));
		...
		// add actions
		$this->dataset->action('Delete','admincp/mailing/delete_emails');
	...
	*/
	function new_email () {
		$data = array(
					...
					'form_action' => site_url('admincp/mailing/new_email_2')
					);
	...
	*/
	function new_email_2 () {
		...
		$data = array(
					...
					'form_action' => site_url('admincp/mailing/post_email/new')
					);
	...
	*/
	function edit_email($id) {
		$this->load->model('mailing/email_model');
		...
		$data = array(
					...
					'form_action' => site_url('admincp/mailing/post_email/edit/' . $email['id'])
					);
	...
	*/
	function email_layout () {
		...
		$data = array(
					'form' => $form->display(),
					'form_action' => site_url('admincp/mailing/post_email_layout')
				);
	...
	*/
	function post_email_layout () {
		...
		redirect('admincp/mailing/email_layout');
	}
}